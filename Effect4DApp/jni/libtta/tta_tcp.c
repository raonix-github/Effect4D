...
static int _tta_set_slave(tta_t *ctx, int slave)
{
    ...
    if (slave >= 0 && slave <= 247) {
        ctx->slave = slave;
    } else if (slave == TTA_TCP_SLAVE) {
        /* The special value TTA_TCP_SLAVE (0xFF) can be used in TCP mode to
...
int _tta_tcp_prepare_response_tid(const uint8_t *pkt, int *pktlen)
{
    return (pkt[0] << 8) + pkt[1];
}

ssize_t _tta_tcp_send(tta_t *ctx, const uint8_t *pkt, int pktlen)
{
    /* MSG_NOSIGNAL
...
int _tta_tcp_receive(tta_t *ctx, uint8_t *pkt, int fgtimeout)
{
    return _tta_receive_pkt(ctx, pkt, fgtimeout);
}

ssize_t _tta_tcp_recv(tta_t *ctx, uint8_t *pkt, int pktlen)
{
    return recv(ctx->s, (char *)pkt, pktlen, 0);
}

int _tta_tcp_check_integrity(tta_t *ctx, uint8_t *pkt, const int pktlen)
{
    return pktlen;
}

int _tta_tcp_pre_check_confirmation(tta_t *ctx, const uint8_t *req,
...
ttamsg_t *_tta_tcp_conv_pkt2ttamsg(tta_t *ctx, uint8_t *pkt, int pktlen)
{
	ttamsg_t *ttamsg;
	ttamsg_tcp_hd_t *hd; 
	int len;
	...
	ttamsg->protoid = ntohs(hd->protoid);
	ttamsg->unitid = ntohs(hd->unitid);
	ttamsg->len = len;
...
int _tta_tcp_conv_ttamsg2pkt(tta_t *ctx, ttamsg_t *ttamsg, uint8_t *pkt)
{
...
void _tta_tcp_close(tta_t *ctx)
{
    shutdown(ctx->s, SHUT_RDWR);
    close(ctx->s);
}

int _tta_tcp_flush(tta_t *ctx)
{
    int rc;
    int rc_sum = 0;
...
int _tta_tcp_select(tta_t *ctx, fd_set *rset, struct timeval *tv, int length_to_read)
{
    int s_rc;
...
tta_t* tta_new_tcp(const char *ip, int port)
{
    ...
    /* Could be changed after to reach a remote serial TTA device */
    ctx->slave = TTA_TCP_SLAVE;
