...
int _tta_rs485_flush(tta_t *);

// 2012.09.07 bygomma : RS485
#include <sys/ioctl.h>
int write_uart(int fd, const uint8_t *buf, int size)
{
...
void _tta_rs485_ioctl_rts(int fd, int on)
{
...
static int _tta_set_slave(tta_t *ctx, int slave)
{
    /* Broadcast address is 0 (TTA_BROADCAST_ADDRESS) */
...
int _tta_rs485_prepare_response_tid(const uint8_t *pkt, int *pktlen)
{
...
ssize_t _tta_rs485_send(tta_t *ctx, const uint8_t *pkt, int pktlen)
{
#if defined(_WIN32)
    ...
    if (ctx_rs485->rts != TTA_RS485_RTS_NONE) {
        ...
        if (ctx->debug) {
            LOGE("Sending request using RTS signal\n");
        }
...
int _tta_rs485_receive(tta_t *ctx, uint8_t *pkt, int fgtimeout)
{
    int rc;
    tta_rs485_t *ctx_rs485 = ctx->backend_data;
...
ssize_t _tta_rs485_recv(tta_t *ctx, uint8_t *pkt, int pktlen)
{
#if defined(_WIN32)
...
/* The check_integrity function shall return 0 is the message is ignored and the
   ...
   errno to EMBADCRC. */
int _tta_rs485_check_integrity(tta_t *ctx, uint8_t *pkt,
                                const int pktlen)
{
	...
	else
	{
        if (ctx->debug)
		{
            LOGE("ERROR SUM received %0X:%0X != SUM calculated %0X:%0X\n",
...
int _tta_rs485_pre_check_confirmation(tta_t *ctx, const uint8_t *req,
                                       const uint8_t *rsp, int rsp_length)
{
    ...
    if (req[1] != 0 && req[1] != rsp[1]) {
        if (ctx->debug) {
            LOGE(				"The responding slave %d isn't the requested slave %d",
				rsp[0], req[0]);
        }
        errno = EMBBADSLAVE;
        return -1;
    ...
    if ((req[1] != rsp[1]) || (req[3] != (rsp[3] & 0x7F)))
	{
        if (ctx->debug)
		{
            LOGE("ERROR The responding ACK is invalid");
        }
        errno = EMBBADSLAVE;
        return -1;
    }
...
ttamsg_t *_tta_rs485_conv_pkt2ttamsg(tta_t *ctx, uint8_t *pkt, int pktlen)
{
	ttamsg_t *ttamsg;
...
int _tta_rs485_conv_ttamsg2pkt(tta_t *ctx, ttamsg_t *ttamsg, uint8_t *pkt)
{
//	tta_rs485_t *ctx_rs485 = ctx->backend_data;
	ttamsg_hd_t *hd;
	int i, datalen;
	...
	for(i=0; i<datalen; i++)
	{
		pkt[5+i] = ttamsg->data[i];
	}
	pkt[5+datalen] = sum_xor(pkt, 5+datalen);
	pkt[6+datalen] = sum_add(pkt, 6+datalen);
...
static int _tta_rs485_connect(tta_t *ctx)
{
    ...
    if (ctx->debug) {
        LOGD("Opening %s at %d bauds (%c, %d, %d)\n",
    ...
    if (ctx_rs485->w_ser.fd == INVALID_HANDLE_VALUE) {
        LOGE("ERROR Can't open the device %s (%s)\n",
    ...
    if (!GetCommState(ctx_rs485->w_ser.fd, &ctx_rs485->old_dcb)) {
        LOGE("ERROR Error getting configuration (LastError %d)\n",
                (int)GetLastError());
    ...
    default:
        dcb.BaudRate = CBR_9600;
        LOGD("WARNING Unknown baud rate %d for %s (B9600 used)\n",
    ...
    if (!SetCommState(ctx_rs485->w_ser.fd, &dcb)) {
        LOGE("ERROR Error setting new configuration (LastError %d)\n",
                (int)GetLastError());
        CloseHandle(ctx_rs485->w_ser.fd);
        ctx_rs485->w_ser.fd = INVALID_HANDLE_VALUE;
        return -1;
    ...
    */
    ...
    default:
        speed = B9600;
        if (ctx->debug) {
            LOGE("WARNING Unknown baud rate %d for %s (B9600 used)\n",
                    ctx_rs485->baud, ctx_rs485->device);
        }
    }
...
#else
        if (ctx->debug) {
            LOGE("This function isn't supported on your platform\n");
        }
        errno = ENOTSUP;
        return -1;
#endif
...
void _tta_rs485_close(tta_t *ctx)
{
    ...
    if (!SetCommState(ctx_rs485->w_ser.fd, &ctx_rs485->old_dcb))
        LOGE("ERROR Couldn't revert to configuration (LastError %d)\n",
                (int)GetLastError());

    if (!CloseHandle(ctx_rs485->w_ser.fd))
        LOGE("ERROR Error while closing handle (LastError %d)\n",
                (int)GetLastError());
#else
...
int _tta_rs485_flush(tta_t *ctx)
{
#if defined(_WIN32)
    tta_rs485_t *ctx_rs485 = ctx->backend_data;
...
int _tta_rs485_select(tta_t *ctx, fd_set *rset,
                       struct timeval *tv, int length_to_read)
{
    int s_rc;
#if defined(_WIN32)
    s_rc = win32_ser_select(&(((tta_rs485_t*)ctx->backend_data)->w_ser),
                            length_to_read, tv);
    if (s_rc == 0) {
        errno = ETIMEDOUT;
        return -1;
    ...
    while ((s_rc = select(ctx->s+1, rset, NULL, NULL, tv)) == -1) {
        if (errno == EINTR) {
            if (ctx->debug) {
                LOGE("A non blocked signal was caught\n");
            }
                         ...
                         int stop_bit)
{
    ...
    if (ret_size == 0) {
        LOGE("The device string is empty\n");
        tta_free(ctx);
        errno = EINVAL;
        return NULL;
    }
    ...
    if (ret_size >= dest_size) {
        LOGE("The device string has been truncated\n");
        tta_free(ctx);
        errno = EINVAL;
        return NULL;
    }
